...
- **Centralize repo URL.** `https://github.com/SpongeBobCodePants/better-hayabusa` is hardcoded in both [src/lib/components/UnsupportedOs.svelte](../../src/lib/components/UnsupportedOs.svelte) and [src/routes/settings/about/+page.svelte](../../src/routes/settings/about/+page.svelte). With a third consumer, extract to `src/lib/constants.ts`.
